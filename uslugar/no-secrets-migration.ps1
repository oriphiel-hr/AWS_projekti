# USLUGAR - Konačno rješenje - koristi postojeći service
# ======================================================

Write-Host "🌱 USLUGAR - Konačno rješenje za kategorije" -ForegroundColor Green
Write-Host "===========================================" -ForegroundColor Green

Write-Host "💡 KORAK 1: Koristi postojeći service koji već radi" -ForegroundColor Yellow
...
if ($serviceStatus -eq "ACTIVE") {
    ...
    $migrationTaskDef = @{
        ...
        family = "uslugar-migration-no-secrets"
    ...
    $migrationTaskDef | ConvertTo-Json -Depth 10 | Out-File -FilePath "no-secrets-task-def.json" -Encoding UTF8
    
    # Registriraj task definition
    $migrationTaskDefArn = aws ecs register-task-definition --cli-input-json file://no-secrets-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text
    
    Write-Host "📝 No-secrets task definition registriran: $migrationTaskDefArn" -ForegroundColor Green
    ...
    $migrationTaskArn = aws ecs run-task `
        ...
        --task-definition uslugar-migration-no-secrets `
        --launch-type FARGATE `
    ...
    Write-Host "✅ No-secrets migration task pokrenut: $migrationTaskArn" -ForegroundColor Green
    
    # Čekaj da se task završi
    Write-Host "⏳ Čekam da se no-secrets migration task završi..." -ForegroundColor Yellow
    ...
    Write-Host "📊 No-secrets migration task status: $migrationStatus" -ForegroundColor Cyan
    ...
    if ($migrationStatus -eq "STOPPED") {
        ...
        Write-Host "❌ No-secrets migration task nije završio uspješno" -ForegroundColor Red
    }
...
Remove-Item "no-secrets-task-def.json" -ErrorAction SilentlyContinue
