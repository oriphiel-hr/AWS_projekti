# USLUGAR - Konačno rješenje - bez secrets-a
# ===========================================

Write-Host "🌱 USLUGAR - Konačno rješenje za kategorije" -ForegroundColor Green
Write-Host "===========================================" -ForegroundColor Green

Write-Host "💡 KORAK 1: Kreiraj task definition bez secrets-a" -ForegroundColor Yellow
Write-Host "================================================" -ForegroundColor Yellow

# Kreiraj task definition bez secrets-a
$finalTaskDef = @{
...
$finalTaskDef | ConvertTo-Json -Depth 10 | Out-File -FilePath "final-task-def.json" -Encoding UTF8

# Registriraj task definition
Write-Host "📝 Registriranje final task definition-a..." -ForegroundColor Yellow
$finalTaskDefArn = aws ecs register-task-definition --cli-input-json file://final-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text
...
Write-Host "✅ Final task definition registriran: $finalTaskDefArn" -ForegroundColor Green
...
Write-Host "🌱 Pokretanje final migracije..." -ForegroundColor Yellow
$migrationTaskArn = aws ecs run-task `
    --cluster apps-cluster `
    --task-definition uslugar-final `
    --launch-type FARGATE `
...
Write-Host "✅ Final migracija task pokrenut: $migrationTaskArn" -ForegroundColor Green

# Čekaj da se task završi
Write-Host "⏳ Čekam da se final migracija završi..." -ForegroundColor Yellow
...
Write-Host "📊 Final migracija status: $migrationStatus" -ForegroundColor Cyan
...
if ($migrationStatus -eq "STOPPED") {
    ...
    Write-Host "❌ Final migracija nije završila uspješno" -ForegroundColor Red
}
