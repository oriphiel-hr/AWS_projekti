# USLUGAR - Kreiranje novog service-a za migraciju
# ================================================

Write-Host "🌱 USLUGAR - Kreiranje novog service-a za migraciju" -ForegroundColor Green
Write-Host "=================================================" -ForegroundColor Green

# Kreiraj novi service
Write-Host "🚀 Kreiranje novog ECS service-a..." -ForegroundColor Yellow
...
# Kreiraj service
Write-Host "📋 Kreiranje service-a..." -ForegroundColor Yellow
$serviceArn = aws ecs create-service `
    --cluster apps-cluster `
    --service-name uslugar-migration `
    --task-definition uslugar-simple:1 `
    --desired-count 1 `
    --launch-type FARGATE `
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Greška pri kreiranju service-a" -ForegroundColor Red
    exit 1
}

Write-Host "✅ Service kreiran: $serviceArn" -ForegroundColor Green

# Čekaj da se service pokrene
Write-Host "⏳ Čekam da se service pokrene..." -ForegroundColor Yellow
aws ecs wait services-stable --cluster apps-cluster --services uslugar-migration

# Provjeri status
$serviceStatus = aws ecs describe-services --cluster apps-cluster --services uslugar-migration --query 'services[0].status' --output text
Write-Host "📊 Service status: $serviceStatus" -ForegroundColor Cyan

if ($serviceStatus -eq "ACTIVE") {
    Write-Host "✅ Service je aktivan!" -ForegroundColor Green
    
    # Pokreni migraciju kroz service
    Write-Host "🌱 Pokretanje migracije kroz service..." -ForegroundColor Yellow
    ...
    $migrationTaskDef | ConvertTo-Json -Depth 10 | Out-File -FilePath "migration-task-def.json" -Encoding UTF8
    
    # Registriraj task definition
    $migrationTaskDefArn = aws ecs register-task-definition --cli-input-json file://migration-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text
    
    Write-Host "📝 Migration task definition registriran: $migrationTaskDefArn" -ForegroundColor Green
    
    # Pokreni task za migraciju
    $migrationTaskArn = aws ecs run-task `
        --cluster apps-cluster `
        --task-definition uslugar-migration `
        --launch-type FARGATE `
        --network-configuration "awsvpcConfiguration={subnets=[$subnetId],securityGroups=[$securityGroupId]}" `
        --overrides '{
            "containerOverrides": [{
