# USLUGAR - Pokreni SQL migraciju kroz ECS task
# =============================================

Write-Host "🌱 USLUGAR - Pokreni SQL migraciju kroz ECS task" -ForegroundColor Green
    ...
    containerDefinitions = @(
        @{
            ...
            command = @(
                ...
                "-U", "uslugar_user",
                "-d", "uslugar",
                "-f", "/sql/add-missing-categories-final.sql"
            )
            logConfiguration = @{
                logDriver = "awslogs"
                options = @{
