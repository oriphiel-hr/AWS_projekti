# USLUGAR - Korištenje postojećeg service-a za migraciju
# ======================================================

Write-Host "🌱 USLUGAR - Korištenje postojećeg service-a za migraciju" -ForegroundColor Green
Write-Host "=======================================================" -ForegroundColor Green

# Pronađi postojeći service
Write-Host "🔍 Pronalazim postojeći USLUGAR service..." -ForegroundColor Yellow

$serviceName = "uslugar-service-2gk1f1mv"
Write-Host "✅ Service pronađen: $serviceName" -ForegroundColor Green

# Provjeri da li service radi
$serviceStatus = aws ecs describe-services --cluster apps-cluster --services $serviceName --query 'services[0].status' --output text
Write-Host "📊 Service status: $serviceStatus" -ForegroundColor Cyan

if ($serviceStatus -eq "ACTIVE") {
    Write-Host "✅ Service je aktivan!" -ForegroundColor Green
    
    # Pokreni migraciju kroz postojeći service
    ...
    $migrationTaskDef = @{
        ...
        family = "uslugar-migration-service"
    ...
    $migrationTaskDef | ConvertTo-Json -Depth 10 | Out-File -FilePath "service-migration-task-def.json" -Encoding UTF8
    
    # Registriraj task definition
    $migrationTaskDefArn = aws ecs register-task-definition --cli-input-json file://service-migration-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text
    
    Write-Host "📝 Service migration task definition registriran: $migrationTaskDefArn" -ForegroundColor Green
    ...
    $migrationTaskArn = aws ecs run-task `
        ...
        --task-definition uslugar-migration-service `
        --launch-type FARGATE `
    ...
    Write-Host "✅ Service migration task pokrenut: $migrationTaskArn" -ForegroundColor Green
    
    # Čekaj da se task završi
    Write-Host "⏳ Čekam da se service migration task završi..." -ForegroundColor Yellow
    ...
    Write-Host "📊 Service migration task status: $migrationStatus" -ForegroundColor Cyan
    ...
    if ($migrationStatus -eq "STOPPED") {
        ...
        Write-Host "❌ Service migration task nije završio uspješno" -ForegroundColor Red
    }
    
} else {
    Write-Host "❌ Service nije aktivan" -ForegroundColor Red
}

# Očisti privremene datoteke
Remove-Item "service-migration-task-def.json" -ErrorAction SilentlyContinue
