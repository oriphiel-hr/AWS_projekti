# USLUGAR - Korištenje postojećeg aktivnog task-a za migraciju
# ============================================================

Write-Host "🌱 USLUGAR - Korištenje postojećeg task-a za migraciju" -ForegroundColor Green
Write-Host "=====================================================" -ForegroundColor Green

# Pronađi aktivan USLUGAR task
Write-Host "🔍 Pronalazim aktivan USLUGAR task..." -ForegroundColor Yellow

$activeTaskArn = "arn:aws:ecs:eu-north-1:666203386231:task/apps-cluster/78658fc9312b479a8aa7cf6584e8c5d1"
Write-Host "✅ Aktivan task pronađen: $activeTaskArn" -ForegroundColor Green

# Provjeri da li task radi
$taskStatus = aws ecs describe-tasks --cluster apps-cluster --tasks $activeTaskArn --query 'tasks[0].lastStatus' --output text
Write-Host "📊 Task status: $taskStatus" -ForegroundColor Cyan

if ($taskStatus -eq "RUNNING") {
    Write-Host "✅ Task je aktivan!" -ForegroundColor Green
    
    # Pokreni migraciju kroz postojeći task
    Write-Host "🌱 Pokretanje migracije kroz postojeći task..." -ForegroundColor Yellow
    
    # Kreiraj novi task definition s migracijom
    $migrationTaskDef = @{
        ...
        family = "uslugar-migration-final"
    ...
    $migrationTaskDef | ConvertTo-Json -Depth 10 | Out-File -FilePath "final-migration-task-def.json" -Encoding UTF8
    
    # Registriraj task definition
    $migrationTaskDefArn = aws ecs register-task-definition --cli-input-json file://final-migration-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text
    
    Write-Host "📝 Final migration task definition registriran: $migrationTaskDefArn" -ForegroundColor Green
    ...
    $migrationTaskArn = aws ecs run-task `
        ...
        --task-definition uslugar-migration-final `
        --launch-type FARGATE `
    ...
    Write-Host "✅ Final migration task pokrenut: $migrationTaskArn" -ForegroundColor Green
    
    # Čekaj da se task završi
    Write-Host "⏳ Čekam da se final migration task završi..." -ForegroundColor Yellow
    ...
    Write-Host "📊 Final migration task status: $migrationStatus" -ForegroundColor Cyan
    ...
    if ($migrationStatus -eq "STOPPED") {
        ...
        Write-Host "❌ Final migration task nije završio uspješno" -ForegroundColor Red
    }
    
} else {
    Write-Host "❌ Task nije aktivan" -ForegroundColor Red
}

# Očisti privremene datoteke
Remove-Item "final-migration-task-def.json" -ErrorAction SilentlyContinue
